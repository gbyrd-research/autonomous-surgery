...
from autonomous_surgery.helpers.common import Logger
...
class MetaWorldDatasetACT(MetaWorldDataset):
    def __init__(
        self,
        chunk_size: int,
        data_dir,
        split: str = None,
        custom_split_size: int = None,
    ):
        super().__init__(data_dir, split, custom_split_size)
        self.chunk_size = chunk_size

    def __len__(self):
        # One sample per timestep (simplest ACT-style indexing)
        return self._dataset_size
    ...
    def __getitem__(self, idx):
        ...
        Returns:
            ...
            action_mask:  (chunk_size,) 1 = valid, 0 = padded
            text:         instruction
        """
        ...
        action_mask = torch.zeros(self.chunk_size, dtype=torch.bool)

        # Find episode end for this index
        episode_end = self._episode_ends[
            torch.searchsorted(
                torch.from_numpy(self._episode_ends), idx, right=True
            ).item()
        ]
        ...
        for i in range(self.chunk_size):
            t = idx + i
            if t < episode_end and t < self._dataset_size:
                actions[i] = torch.from_numpy(self._actions[t]).float()
                action_mask[i] = True
            else:
                break

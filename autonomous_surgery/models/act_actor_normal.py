# autonomous_surgery/models/act_actor_normal.py
# -*- coding: utf-8 -*-
"""
autonomous_surgery + ACT (DETR-VAE style) actor (with conditional prior) AND Auto-Normalization.
...
- If autonomous_surgery encoder returns tokens/global -> use them.
...
class autonomous_surgeryActActor(nn.Module):
    def __init__(
        self,
    ...
    # ---------------------------------------------------------------------
    # autonomous_surgery encoder token extraction
    # ---------------------------------------------------------------------
    ...
    def _autonomous_surgery_encode_tokens(
        self,
        images: Tensor,
        point_clouds: Tensor,
        robot_states: Tensor,
        ...
        if tokens is None or global_feat is None:
            raise RuntimeError("Failed to get tokens/global from autonomous_surgery encoder output or hook.")
    ...
    def forward(
        ...
        tokens_d, global_d = self._autonomous_surgery_encode_tokens(images, point_clouds, robot_states_norm, texts)
        bs = tokens_d.shape[0]

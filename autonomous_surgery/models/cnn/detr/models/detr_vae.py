        ...
        if backbones is not None:
            ...
            for backbone in backbones:
                down_proj = nn.Sequential(
                    nn.Conv2d(backbone.num_channels, 128, kernel_size=3, padding=1),
                    nn.ReLU(),
            ...
            mlp_in_dim = 784 * (len(backbones) + 1) + state_dim
            self.mlp = mlp(
                input_dim=mlp_in_dim, hidden_dim=1024, output_dim=state_dim, hidden_depth=2
            )
        else:
            raise NotImplementedError

    def forward(self, qpos, images, env_state, actions=None):
        """
        qpos: batch, qpos_dim
        images: dict: {img_name: Tensor(batch, channel, height, width)}
        env_state: None
        actions: batch, seq, action_dim
        """
        is_training = actions is not None  # train or val
        ...
        all_cam_features = []
        for cam_id, cam_name in enumerate(self.camera_names):
            if cam_name not in images.keys():
                raise Exception(f"Cam name {cam_name} not found in images keys. Please make sure your specified cam names are provided in the dataset images variable as a image name.")
            features, pos = self.backbones[cam_id](images[cam_name])
            features = features[0]  # take the last layer feature
            pos = pos[0]  # not used

...
# This file defines the point grouping, local geometry aggregation, and encoder
# blocks used by the Lift3D point cloud backbone.
...
def square_distance(src, dst):
    ...
    B, N, _ = src.shape
    _, M, _ = dst.shape
    # Use the expansion of ||a-b||^2 = ||a||^2 + ||b||^2 - 2*a^T*b
...
def index_points(points, idx):
    ...
    repeat_shape[0] = 1
    # Build a batch index tensor so we can gather points per batch.
    batch_indices = (
...
def knn_point(nsample, xyz, new_xyz):
    ...
    """
    # Compute pairwise distances from each query point to all points.
...
class FPS_kNN(nn.Module):
    ...
    def forward(self, xyz, x):
        ...
        # Farthest Point Sampling (FPS) picks representative centers.
        # NOTE: The FPS result is overwritten by a random index below, which
        # makes grouping stochastic. This is likely intentional for ablations.
...
class LGA(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.type = type
        # Positional encoding for local geometry (xyz -> sinusoidal features).
        ...
        # Stack of residual 2D conv blocks applied after geometry encoding.
        self.linear2 = []
    ...
    def forward(self, lc_xyz, lc_x, knn_xyz, knn_x):
        ...
        # Normalize local coordinates differently per dataset type.
        if self.type == "mn40":
            ...
            # Center each neighborhood and scale by max absolute coordinate.
        ...
        # Feature Expansion
        B, G, K, C = knn_x.shape
        # Concatenate center features to each neighbor feature.

        knn_x = torch.cat([knn_x, lc_x.reshape(B, G, 1, -1).repeat(1, 1, K, 1)], dim=-1)
        # Reorder to channel-first for Conv2d processing.
        knn_xyz = knn_xyz.permute(0, 3, 1, 2)
        ...
        # Residual Conv2d blocks refine the local features.
...
class Pooling(nn.Module):
    ...
    def forward(self, knn_x_w):
        # Max-pool over neighbors to aggregate a local feature per group.
...
class Linear2Layer(nn.Module):
    ...
    def forward(self, x):
        # Residual bottleneck: Conv2d -> Conv2d + skip connection.
...
class PosE_Geo(nn.Module):
    ...
    def forward(self, knn_xyz, knn_x):
        ...
        # Scale coordinates with exponential frequencies, then apply sin/cos.
        ...
        # Arrange as [B, C_out, G, K] to match knn_x layout.
...
class EncP(nn.Module):
    def __init__(
        ...
        # Raw-point Embedding: project input features to embed_dim.
        self.raw_point_embed = Linear1Layer(in_channels, self.embed_dim, bias=False)

        # Per-stage modules: grouping, local aggregation, and pooling.
        ...
        # Multi-stage Hierarchy: halve points and expand channels each stage.
    ...
    def forward(self, xyz, x):
        ...
        for i in range(self.num_stages):
            # FPS, kNN: select centers and gather neighborhoods.
...
class Point_PN_scan(nn.Module):
    def __init__(
        ...
        super().__init__()
        # Parametric Encoder (hierarchical point feature extractor).
        self.EncP = EncP(
            in_channels,
            input_points,
            num_stages,
